...
def resource_path(relative_path):
    """Dùng cho cả khi chạy script và chạy .exe"""
    try:
        base_path = sys._MEIPASS  # PyInstaller tạm giải nén tại đây
    except AttributeError:
        base_path = os.path.abspath(".")

    return os.path.join(base_path, relative_path)

icon_path = resource_path("rs232UsbIcon.ico")
root.iconbitmap(default=icon_path)

def create_icon_image():
    path = resource_path("rs232UsbIcon.png")
    return Image.open(path).resize((32, 32), Image.Resampling.LANCZOS)
# def create_icon_image():
#     image = Image.new('RGB', (64, 64), color='white')
#     draw = ImageDraw.Draw(image)
#     draw.rectangle((16, 16, 48, 48), fill='blue', outline='black')
#     draw.text((20, 20), "COM", fill='white')
#     return image
...
def show_tray_icon():
    ...
    def restore_app(icon, item=None):
        root.after(0, lambda: [root.deiconify(), icon.stop()])

    def exit_app(icon, item=None):
        stop_reading()
        root.after(0, root.destroy)
        icon.stop()
    image = create_icon_image()  # Hoặc dùng Image.open("icon.png")
    tray_icon = pystray.Icon("UPAQSerialTool", image, "UPA-Q Tool", menu=pystray.Menu(
        pystray.MenuItem("Hiện lại cửa sổ", restore_app),
        pystray.MenuItem("Thoát", exit_app)
    ))
    tray_icon.on_activate = restore_app
    tray_icon.run()
